...
'''def sendMessage(outboundNumber,message_body):
...
def outgoingCall(outboundNumber,message_body):
    ...
    call = client.calls.create(to=to_number,
                               from_=from_number,
                               url = url + urlencode({'Message': message_body}))
    return call.sid'''
def sendMessage(outboundNumber,message_body):
    client = Client(account_sid, auth_token)

    #message_body = "Hey it works"
    message = client.messages.create(
                                  body=message_body,
                                  from_='7315954188',
                                  to=outboundNumber)
...
def outgoingCall(outboundNumber):
